...
# i actually dont know how i started this - is it like a podman containe
node=[{"ws_endpoint":"ws://localhost:3301/", "tasks":[], "browser": None}]
print(node[0]["ws_endpoint"])
...
async def main():
    async with async_playwright() as p:
        browser = await p.webkit.connect(endpoint=node[0]["ws_endpoint"])  
        print("browser openeyed")
        for task in queue:
            print(f"running a tasky: {task}")
            run_scan_task()
